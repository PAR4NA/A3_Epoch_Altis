...
[] spawn {	waitUntil{player == player && !isNil "SEM_version"}; sleep 50;
	...
	titleText [format["\nBem Vindo ao Survivors BR Epoch %1 sobrevivente %2.", _worldName, name player], "PLAIN DOWN", 2];
	sleep 15;
	titleText [format["tecla 9 oculta icones HUD; telcla 8 abre configuração do HUD"], "PLAIN DOWN", 4];
	/* End of advertisement */
	"SEM_version" addPublicVariableEventHandler {titleText [format["tecla 9 oculta icones HUD; telcla 8 abre configuração do HUD"", _this select 1], "PLAIN DOWN"]};
};	
